...
describe( 'myrecipes.com', () => {
  ...
  beforeAll( async () => {
    const source = await testAsset('www.myrecipes.com/RoastedRedPepperHummusVeggieWrapsRecipe.html')
  ...
  test( 'description', async () => {
    expect( recipe.description ).toBe( 'Lunch to go never looked so good. Try these portable veggie wraps with any flavor of hummus you like and whatever crisp veggies you have on hand.' )
  } )

  test( 'image_url', async () => {
    expect( recipe.image_url ).toMatch( /roasted-red-pepper-hummus-veggie-wraps-ck.jpg/ )
  } )
  ...
  test( 'procedure_lists', async () => {
    ...
    expect( recipe.procedure_lists[0].lines[0].text ).toMatch( /^Spread about 1\/3 cup hummus over each tortilla/ )
  } )
  ...
  test( 'ingredient_lists', async () => {
    ...
    expect( recipe.ingredient_lists[0].lines[0].name ).toBe( 'prepared roasted red pepper hummus' )
